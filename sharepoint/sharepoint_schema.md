...
이 파일은 시작실 작업 현황판에서 사용하는 SharePoint 리스트의 스키마를 정의합니다. 앱을 배포하기 전에 아래와 동일한 컬럼을 가진 리스트를 생성해야 합니다. 데이터 소스 이름은 다음과 같습니다.

- WorkRequests_DB (das): 작업 의뢰의 기본 정보를 저장합니다.
- WorkRequestComments_DB: 각 작업 의뢰에 대한 댓글 및 이력을 기록합니다.
...
- UserInfo_DB와 Project_DB는 별도로 생성하여 의뢰자 정보와 프로젝트 마스터 데이터를 저장합니다. Project_DB는 프로젝트명, 업무 계정 등을 포함하며 의뢰 등록 시 선택 목록으로 사용합니다.
- 앱 코드에서 관리자 전용 조회(varIsViewer)를 구현했으므로, 리스트 자체에는 추가적인 권한 컬럼이 필요하지 않습니다.
